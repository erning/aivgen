...
class _FakeProvider:
    ...
    def chat_completions(
        ...
        if kwargs.get("stream") is True:
            return iter(
                [
                    _Chunk(choices=[_ChunkChoice(delta=_Delta(content="he"))]),
                    _Chunk(
                        choices=[
                            _ChunkChoice(
                                delta=_Delta(content="llo", reasoning_content="think")
                            )
                        ]
                    ),
                    _Chunk(
...
def test_provider_chat_streams_by_default(monkeypatch) -> None:  # noqa: ANN001
    fake = _FakeProvider()
